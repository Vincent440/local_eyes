...
function census (zip) {
  ...
  var queryURL =
    'https://api.census.gov/data/2017/acs/acs5/profile?get=NAME' +
    varString +
        ...
        if (response[1][i] < 0) {
          ...
          if (response[0][i].slice(-2) === 'PE') {
            if (response[0][i] === 'DP02_0060PE') {
              const hsGradRate = 100 - response[1][i]
              $('#czip' + a).text(hsGradRate + '%')
            } else {
              $('#czip' + a).text(response[1][i] + '%')
            }
          } else if (
            response[0][i] === 'DP05_0001E' ||
            response[0][i] === 'DP03_0062E' ||
            response[0][i] === 'DP03_0063E' ||
            response[0][i] === 'DP04_0089E' ||
            response[0][i] === 'DP04_0134E'
          ) {
